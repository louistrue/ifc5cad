...
function selectExportFormat(host: HTMLElement): Promise<"IFC" | "IFCX" | undefined> {
    return new Promise((resolve) => {
        ...
        picker.style.border = "1px solid #c9c9c9";
        picker.style.borderRadius = "6px";
        picker.style.padding = "6px";
        picker.style.boxShadow = "0 4px 14px rgba(0,0,0,0.18)";
        ...
        select.onchange = () => cleanup(select.value === "IFCX" ? "IFCX" : "IFC");
        cancel.onclick = () => cleanup(undefined);

        picker.append(select, cancel);
        host.append(picker);
